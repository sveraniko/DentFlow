...
def test_non_ready_admin_actions_do_not_trigger_pickup_ready_delivery() -> None:
    repo = InMemoryCareRepo()
    delivery = _FakePickupReadyDelivery()
    service = CareCommerceService(repo, patient_order_delivery=delivery)
    product = asyncio.run(service.create_or_update_product(clinic_id="c1", sku="SKU-1", title_key="care.product.aftercare_brush.title", description_key=None, category="hygiene", price_amount=1000, currency_code="RUB", status="active"))
    ...
    updated = asyncio.run(service.apply_admin_order_action(care_order_id=order.care_order_id, action="pay_required"))

    assert updated is not None and updated.status == "awaiting_payment"
    assert delivery.calls is None


def test_repeated_ready_on_already_ready_order_does_not_duplicate_pickup_delivery() -> None:
    repo = InMemoryCareRepo()
    delivery = _FakePickupReadyDelivery()
    service = CareCommerceService(repo, patient_order_delivery=delivery)
    product = asyncio.run(service.create_or_update_product(clinic_id="c1", sku="SKU-1", title_key="care.product.aftercare_brush.title", description_key=None, category="hygiene", price_amount=1000, currency_code="RUB", status="active"))
    ...
    first = asyncio.run(service.apply_admin_order_action(care_order_id=order.care_order_id, action="ready"))
    second = asyncio.run(service.apply_admin_order_action(care_order_id=order.care_order_id, action="ready"))

    assert first is not None and first.status == "ready_for_pickup"
    assert second is not None and second.status == "ready_for_pickup"
    assert delivery.calls is not None and len(delivery.calls) == 1
